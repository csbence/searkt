...
import edu.unh.cs.searkt.experiment.terminationCheckers.InsufficientTerminationCriterionException
...
class RealTimeAStarPlanner<StateType : State<StateType>>(val domain: Domain<StateType>, var configuration: ExperimentConfiguration) : RealTimePlanner<StateType>() {
    ...
    private fun evaluateSuccessors(successors: List<SuccessorBundle<StateType>>, terminationChecker: TerminationChecker): List<SuccessorHeuristicPair<StateType>> {
        var successorHeuristicPairs = heuristicLookahead(successors, 0, terminationChecker)
                ?: throw InsufficientTerminationCriterionException("Not enough time to calculate the successor f values.")

        for (depth in 1..depthLimit.toInt()) {
            successorHeuristicPairs = heuristicLookahead(successors, depth, terminationChecker)
