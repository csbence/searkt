...
import edu.unh.cs.searkt.util.ErrorEstimator;
import edu.unh.cs.searkt.util.ErrorTraceable;
...
import edu.unh.cs.searkt.util.search.SearchResultImpl;
import edu.unh.cs.searkt.util.search.SearchResultImpl.SolutionImpl;
...
public class ExplicitEstimationSearchHFix<StateType extends State<StateType>> extends OfflinePlanner<StateType> {
    ...
    public ExplicitEstimationSearchHFix(Domain<StateType> domain, ExperimentConfiguration configuration) {
    ...
    public ExplicitEstimationSearchHFix(double weight) {
        this.weight = weight;
    }
    ...
    private Node selectNode() {
        ...
        // best dhat
        if (bestDHat.f <= weight * bestF.f) {
            value = gequeue.pollFocal();
            cleanup.remove(value);
        }
        // best fhat
        else if (bestFHat.f <= weight * bestF.f) {
