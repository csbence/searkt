...
//            ,Domains.RACETRACK to "input/racetrack/hansen-bigger-octa.track"
    ...
    domains += (0..1).map { Domains.PANCAKE to "input/pancake/$it.pqq" }

    domains += (1..2).map { Domains.SLIDING_TILE_PUZZLE_4 to "input/tiles/korf/4/real/$it" }
    domains += (1..2).map { Domains.SLIDING_TILE_PUZZLE_4_INVERSE to "input/tiles/korf/4/real/$it" }
    domains += (1..2).map { Domains.SLIDING_TILE_PUZZLE_4_HEAVY to "input/tiles/korf/4/real/$it" }
...
//    domains += (0..10).map { Domains.GRID_WORLD to "input/vacuum/orz100d/orz100d.map_scen_$it" }
    ...
    val domainExtras = listOf(
            ...
            Triple(Domains.GRID_MAP, Configurations.DOMAIN_SEED.toString(), 0..2000L step 200)
    )
...
//            ),
    ...
    configurations = configurations.cartesianProduct(Configurations.WEIGHT.toString(), listOf(10.0))
