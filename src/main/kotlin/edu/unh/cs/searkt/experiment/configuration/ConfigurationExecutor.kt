...
//    }
    ...
    private fun <StateType : State<StateType>> executeDomain(configuration: ExperimentConfiguration, domain: Domain<StateType>, initialState: StateType): ExperimentResult {
        ...
        return when (Planners.valueOf(algorithmName)) {
            ...
            BACK_ES -> executeRealTimeSearch(BackwardEnvelopeSearch(domain, configuration), configuration, domain, sourceState)
            BI_ES -> executeRealTimeSearch(BidirectionalEnvelopeSearch(domain, configuration), configuration, domain, sourceState)
