...
public class ExplicitEstimationSearchHFixCP<StateType extends State<StateType>> extends ClassicalPlanner<StateType> {
...
//    private OpenNodeComparator openComparator = new OpenNodeComparator();
    private BinHeap<Node> open = new BinHeap<>(new FocalNodeComparator(), FOCAL_ID);
//    private GEQueue<Node> gequeue = new GEQueue<>(openComparator,
//            new GENodeComparator(), new FocalNodeComparator(), FOCAL_ID);

    public ExplicitEstimationSearchHFixCP(Domain<StateType> domain, ExperimentConfiguration configuration) {
    ...
    public ExplicitEstimationSearchHFixCP(double weight) {
        this.weight = weight;
    }
...
//        gequeue.updateFocal(null, initNode, 0);
        ...
        while (!open.isEmpty()) {
//            Node oldBest = open.peekOpen();
            Node oldBest = open.peek();
            Node n = selectNode();
            if (n == null) {
                break;
            }
...
//        public int compare(final Node a, final Node b) {
//            if (a.f < weight * b.f) return -1;
//            else if (a.f > weight * b.f) return 1;
//            return 0;
//        }
//    }
