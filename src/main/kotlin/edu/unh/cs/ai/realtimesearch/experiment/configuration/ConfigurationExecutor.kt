...
//    }
    ...
    private fun <StateType : State<StateType>> executeDomain(configuration: ExperimentConfiguration, domain: Domain<StateType>, initialState: StateType): ExperimentResult {
        ...
        return when (Planners.valueOf(algorithmName)) {
            ...
            EESJV -> executeOfflineSearch(ExplicitEstimationSearchH(domain, configuration), configuration, domain, sourceState)
            EESKT -> executeOfflineSearch(ExplicitEstimationSearch(domain, configuration), configuration, domain, sourceState)
            EESFC -> executeOfflineSearch(ExplicitEstimationSearchHFixC(domain, configuration), configuration, domain, sourceState)
            EESFCP -> executeOfflineSearch(ExplicitEstimationSearchHFixCP(domain, configuration), configuration, domain, sourceState)
            EESF -> executeOfflineSearch(ExplicitEstimationSearchHFix(domain, configuration), configuration, domain, sourceState)
