...
class ExplicitEstimationSearchBug<StateType : State<StateType>>(val domain: Domain<StateType>, val configuration: ExperimentConfiguration) : OfflinePlanner<StateType>() {
...
//        }
        when {
            lhs.fHat >= rhs.fHat && lhs.f == rhs.f && lhs.d == rhs.d -> (lhs.h - rhs.h).toInt()
            lhs.fHat >= rhs.fHat && lhs.f == rhs.f -> (lhs.d - rhs.d).toInt()
            lhs.fHat >= rhs.fHat -> (lhs.f - rhs.f).toInt()
            else -> (lhs.fHat - rhs.fHat).toInt()
        }
    }
