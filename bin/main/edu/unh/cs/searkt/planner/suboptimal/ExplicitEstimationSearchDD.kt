...
import edu.unh.cs.searkt.util.collections.gequeue.GEQueue
import edu.unh.cs.searkt.util.collections.rbtree.RBTreeElement
import edu.unh.cs.searkt.util.collections.rbtree.RBTreeNode
import java.util.*
import kotlin.Comparator

class ExplicitEstimationSearchDD<StateType : State<StateType>>(val domain: Domain<StateType>, val configuration: ExperimentConfiguration) : OfflinePlanner<StateType>() {
    ...
    inner class Node<out StateType : State<StateType>>(val state: StateType, var heuristic: Double, var cost: Double,
        ...
        override fun toString(): String {
            return "ExplicitEstimationSearch.Node(state=$state, heuristic=$heuristic, cost=$cost, actionCost=$actionCost, " +
    ...
    private fun insertNode(node: Node<StateType>, oldBest: Node<StateType>, duplicateNode: Boolean = false) {
        if (!duplicateNode) {
    ...
    private fun selectNode(): Node<StateType> {
        ...
        when {
            ...
            bestFHat != null && bestFHat.fHat <= weight * bestF.f -> {
                ...
                value = cleanup.poll()
                // remove only what is in the gequeue
                // sometimes the node will only be in
                // the A* queue since we are dropping duplicates
                if (value.node != null) {
    ...
    private fun fixedSelectNode(): Node<StateType> {
        val value: Node<StateType>?
        val bestDHat = gequeue.peekFocal()
        val bestFHat = gequeue.peekOpen()
        val bestF = cleanup.peek()
    ...
    private fun tentacleSelectNode(): Node<StateType> {
        val value: Node<StateType>?
        val bestDHat = gequeue.peekFocal()
        val bestFHat = gequeue.peekOpen()
        val bestF = cleanup.peek()
            ...
            if (isDuplicateNode) {
                if (successorNode.cost > successorGValueFromCurrent) {
                    ...
                    if (successorNode.getIndex(CLEANUP_ID) != -1) {
                        // gequeue could not have the node
                        // since we are dropping duplicates
                        if (successorNode.node != null) {
                            gequeue.remove(successorNode)
                        }
                        cleanup.remove(successorNode)
                        closed.remove(successorNode.state)
                    }
                    insertNode(successorNode, oldBest, duplicateNode = true)
                }
            } else {
                if (successorNode.cost > successorGValueFromCurrent) {
                    successorNode.apply {
    ...
    override fun plan(state: StateType, terminationChecker: TerminationChecker): List<Action> {
        ...
        while (cleanup.isNotEmpty && !terminationChecker.reachedTermination()) {
            val oldBest = gequeue.peekOpen() ?: cleanup.peek()
